...
public class ShipmentScenario : XceptoScenario
{
    private IContainer? _api;
    public Uri ApiAddress { get; private set; } = new Uri("http://localhost:8080");
    ...
    protected override ScenarioInitialization Initialize(ScenarioInitializationBuilder builder) => builder
        .Do(async () =>
        {
            ...
            ApiAddress = new Uri($"http://localhost:{_api.GetMappedPublicPort(8080)}");
        })
        .Build();
